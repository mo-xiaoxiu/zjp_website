...
一开始我想使用gdb来直接调试以上程序，发现当我打断点在函数`getInstance`中的`static Singleton ms`的时候，会自动跳过该行，把断点打在了`return ms`这一行，那么这样运行时就会观察不到。
...
可以看到在这里`ms`的地址变成了`0x555555558152`。这是因为在`objdump`中观察到的地址是**相对虚拟地址**，是假设从地址0开始加载时的**偏移量**，在ELF中是静态确定的。`0x555555558152`是运行时的实际地址。
...
我在这里打算使用另外一种方式，不使用gdb来查看汇编内容了：使用编译参数和选项来生成**带有详细注释的汇编代码**，方便我们浏览：
...
大家可以自行在自己的linux环境中试验，这里我就不将全部文件内容贴上来了，我们关注下初始化的过程即可：
...
我使用objdump查看`__cxa_guard_acquire`的跳转是如何实现的：
